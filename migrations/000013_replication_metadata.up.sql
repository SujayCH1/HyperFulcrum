-- A node is a physical PostgreSQL instance. Its role describes the role that
-- HyperFulcrum intends that instance to perform.
...
-- A shard has a stable identity independent of whichever physical node is its
-- current desired primary. Standbys remain represented by node_topology.
CREATE TABLE shards (
    id UUID PRIMARY KEY,
    ...
    CONSTRAINT shards_project_id_id_unique
        UNIQUE (project_id, id),
    CONSTRAINT shards_project_id_id_primary_unique
        UNIQUE (project_id, id, primary_node_id),
    CONSTRAINT shards_project_name_unique
        UNIQUE (project_id, shard_name),
    CONSTRAINT shards_project_index_unique
        UNIQUE (project_id, shard_index),
    CONSTRAINT shards_primary_node_unique
        UNIQUE (primary_node_id),
    CONSTRAINT shards_index_nonnegative
        CHECK (shard_index >= 0),
    CONSTRAINT shards_topology_generation_positive
        CHECK (topology_generation > 0),
    CONSTRAINT fk_shards_project
        FOREIGN KEY (project_id)
        REFERENCES projects(id)
        ON DELETE CASCADE,
    CONSTRAINT fk_shards_primary_node
        FOREIGN KEY (project_id, primary_node_id)
        REFERENCES nodes(project_id, id)
);

-- Existing primary nodes become stable logical shards. The deterministic UUID
-- keeps the migration independent of UUID extensions while remaining distinct
-- from the physical node UUID.
INSERT INTO shards (
    id,
    project_id,
    shard_name,
...
ADD CONSTRAINT node_topology_shard_replica_unique
    UNIQUE (shard_id, replica_node_id),
ADD CONSTRAINT node_topology_promotion_priority_nonnegative
...
-- This table stores only the latest agent observation. Historical samples
-- belong in operation events or the monitoring/telemetry subsystem.
CREATE TABLE node_runtime_state (
    ...
    CONSTRAINT fk_node_runtime_state_node
        FOREIGN KEY (node_id)
        REFERENCES nodes(id)
        ON DELETE CASCADE
);
