...
BEGIN
    IF EXISTS (SELECT 1 FROM nodes WHERE node_type = 'unassigned') THEN
        RAISE EXCEPTION 'cannot roll back while unassigned nodes exist';
    END IF;
END
$$;
...
DROP CONSTRAINT IF EXISTS fk_node_topology_logical_shard_primary,
DROP CONSTRAINT IF EXISTS fk_node_topology_logical_shard,
...
ALTER TABLE nodes ALTER COLUMN node_type TYPE ntype USING
    (CASE node_type::TEXT
